import { strapi, type API, type Config } from '@strapi/client';
...
export type GetAttributes<TContentTypeUID extends UID.ContentType> = Modules.Documents.GetValues<TContentTypeUID> & API.Document;
...
export const createTypedStrapiClient = (config: Config) => {
  ...
  return {
    ...
    single: <TContentTypeUID extends UID.ContentType>(resource: TContentTypeUID, options?: SingleCollectionOptions) => {
      ...
      return {
        find: <const TParams extends GetQueryParams<TContentTypeUID>>(queryParams?: TParams) => {
          return clientCollection.find(queryParams as API.BaseQueryParams) as Promise<DocumentResponse<Modules.Documents.Result<TContentTypeUID, TParams>>>;
        },
        update: <const TParams extends GetQueryParams<TContentTypeUID>>(document: Partial<GetAttributes<TContentTypeUID>>, queryParams?: TParams) => {
          return clientCollection.update(document, queryParams as API.BaseQueryParams) as Promise<DocumentResponse<Modules.Documents.Result<TContentTypeUID, TParams>>>;
        },
        delete: <const TParams extends GetQueryParams<TContentTypeUID>>(queryParams?: TParams): Promise<void> => {
          return clientCollection.delete(queryParams as API.BaseQueryParams)
        },
      }
    },

    collection: <TContentTypeUID extends UID.ContentType>(resource: TContentTypeUID, options?: ClientCollectionOptions) => {
      const [, resourceInfo] = resource.split('::') as [string, string];
      const [, contentTypeName] = resourceInfo.split('.') as [string, string];
      const clientCollection = client.collection(pluralize(contentTypeName), options);
