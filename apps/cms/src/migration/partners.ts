...
export const migratePartners = async () => {
  ...
  for (const partner of partners.data) {
    try {
      ...
      const existingOrg = await strapi
        .documents("plugin::better-auth.organization")
        .findFirst({
          filters: { slug: partner.attributes.slug },
        });
      ...
      failed++;
      strapi.log.error(
        `Error migrating partner ${partner.attributes.slug}:`,
        error,
      );
    }
  }
  ...
  strapi.log.info(
    `Partners migration finished. Migrated: ${migrated}, Skipped: ${skipped}, Failed: ${failed}`,
  );
};
